...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Validación 1: Solo criaturas pueden tener ataque o defensa
    if ($tipo_carta != "Criatura" && ($ataque != null || $defensa != null)) {
        echo "❌ Error: Solo las cartas de tipo 'Criatura' pueden tener ataque o defensa.";
        exit;
    }

    // Validación 2: Solo criaturas pueden tener tipos de criatura
    if ($tipo_carta != "Criatura" && ($tipo_criatura != null || $tipo_criatura2 != null)) {
        echo "❌ Error: Solo las cartas de tipo 'Criatura' pueden tener tipos de criatura.";
        exit;
    }

    // Validación 3: Las criaturas deben tener al menos un tipo de criatura
    if ($tipo_carta === "Criatura" && empty($tipo_criatura)) {
        echo "❌ Error: Las cartas de tipo 'Criatura' deben tener al menos un tipo de criatura.";
        exit;
    }
    ...
    // Asignar tipo de carta numérico
    $tipoCartaNumerico = 0;
    ...
    if ($stmt->execute()) {
        ...
        if ($tipoCartaNumerico == 1) {
            ...
            if ($tipo_criatura != $tipo_criatura2 && !empty($tipo_criatura2)) {
